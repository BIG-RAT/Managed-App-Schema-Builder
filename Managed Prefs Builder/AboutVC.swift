...
class AboutVC: NSViewController {
    ...
    let supportText = """
    The Managed App Schema Builder application helps to create a JSON file that can be used to set application preferences. The file is uploaded to a Jamf Pro configuration profile containing the Applications & Custom Settings payload.
    ...
    Warning: Failure to test Managed App Schema Builder thoroughly on non production devices could result in misconfigurations on production devices.
    ...
    func formattedText() -> NSAttributedString {
        ...
        if issuesRange.location != NSNotFound {
            feedbackString.addAttribute(NSAttributedString.Key.link, value: "https://github.com/BIG-RAT/Managed-App-Schema-Builder/issues", range: issuesRange)
        }
